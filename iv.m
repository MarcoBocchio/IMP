function [inputRes, membraneTau, membraneCapacitance, spikeThreshold, firstSpikeAmpl, spikeHalfWidth, fAHPampl, fileName] = iv(fileFormat);
%%
%OPERATION
...
%% Trace trimming
YtraceTrimmed = Ytrace(1:(base+currInjDuration)*Fs,:);
...
sweepsWithSpikes = zeros(1,size(Ytrace,2)); %preallocation
    ...
    for i = 1 : size(Ytrace,2);
       [spikePeaks_i,spikePeaksLocs_i] = findpeaks(YtraceTrimmed(:,i),'MinPeakHeight',SpikeThreshold); %spike peak detection in every sweep
